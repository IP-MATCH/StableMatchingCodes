	...
	try{
		...
		for (int i = 0; i < allo.nbDoctors; i++){
			...
			for (int j = 0; j<allo.doctors[i].nbPref; j++){
				...
				model.addConstr(allo.hospitals[allo.doctors[i].preferences[j] - 1].cap * ( 1 - leftside) - fillingOfHospitalJUpToRankK[allo.doctors[i].preferences[j] - 1][allo.doctors[i].ranks[j]-1] <= 0);
			}	
		} 
